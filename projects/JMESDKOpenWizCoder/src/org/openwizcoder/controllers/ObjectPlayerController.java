...
public class ObjectPlayerController extends BaseController {
    ...
    public ObjectPlayerController() { 
        System.out.print("\nINIT CONTORLER:"+enabled);
    }
    
    public ObjectPlayerController(Spatial sptl) {
        spatial = sptl;
    ...
    public void update(float tpf) {
        if (enabled && spatial != null) {            
            if(OpenWizCoderApp.NETWORKTYPE == "CLIENT"){
                if(smobjshare !=null){
            ...
            if(OpenWizCoderApp.NETWORKTYPE == "SERVER"){
                if(charinput !=null){
                    Vector3f pos = spatial.getLocalTranslation();
                    Vector3f posmove = Vector3f.ZERO;
                     ...
                     if(((pos.x + posmove.x) != pos.x )||((pos.y + posmove.y) != pos.y )||((pos.z + posmove.z) != pos.z ) ){
                         //System.out.print("\n"+spatial.getLocalTranslation()+"update!");

                         if(getApp() !=null){
                             smobjshare.x = pos.x + posmove.x;
                             smobjshare.y = pos.y + posmove.y;
                             smobjshare.z = pos.z + posmove.z;
                             if(getApp().myServer.isRunning()){
                                getApp().myServer.broadcast(smobjshare);
                             }
                         }                     
                     }
    ...
    public Control cloneForSpatial(Spatial _spatial) {
        ObjectPlayerController control = new ObjectPlayerController(_spatial);
